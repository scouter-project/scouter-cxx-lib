...
void data_output::write_uint64(uint64_t value) {
	...
	if(conf->is_lendian) {
		uint64_t temp = util::htonll2(value);
...
void data_output::write_bytes(const char* bytes,int size) {
	check(size);
...
void data_output::write_blob(const char* bytes, int size) {
